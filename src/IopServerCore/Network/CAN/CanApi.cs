...
namespace IopServerCore.Network.CAN
{
  /// <summary>
  ...
  public class CanApi
  {
    ...
    private static Logger log = new Logger("IopServerCore.Network.CAN.CanApi");
    ...
    /// </summary>
    /// <param name="CanEndPoint">IP address and port on which CAN server operates.</param>
    /// <param name="ShutdownSignaling">Owning component's shutdown signaling.</param>
    public CanApi(IPEndPoint CanEndPoint, ComponentShutdown ShutdownSignaling)
    {
      apiUrl = string.Format("http://{0}:{1}/api/v0/", CanEndPoint.Address, CanEndPoint.Port);
  ...
  {
    private static Logger log = new Logger("IopServerCore.Network.CAN.CanUploadResult");
  ...
  {
    private static Logger log = new Logger("IopServerCore.Network.CAN.CanDeleteResult");
  ...
  {
    private static Logger log = new Logger("IopServerCore.Network.CAN.CanRefreshIpnsResult");
